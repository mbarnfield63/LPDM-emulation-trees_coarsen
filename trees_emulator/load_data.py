...
class LoadData:
    ...
    inputs:
        ...
        - coarsen_factor: reduce the initial resolution by a factor. If empty, initial resolution is used.
    ...
    """
    def __init__(self, year, site = "MHD", domain=None, siteheight = None, metheight = None, size=10, coarsen_factor=1.0, verbose = False, met_datadir = None, extramet_datadir = None, fp_datadir = None):
        ...
        # v8 vroom vroom
        if coarsen_factor != 1:
            self.fp_data_full = pad_and_coarsen_dataset(self.fp_data_full, coarsen_factor)
            self.met = pad_and_coarsen_dataset(self.met, coarsen_factor)
...
def slice_around_index_skip(arr, index, skip):
    sliced_before = arr[index-1::-skip][::-1] if index > 0 else []
    sliced_after = arr[index::skip]
    return np.concatenate((sliced_before, sliced_after))
...
def cut_data(fp_full, release_lat, release_lon, size, returnlatlons = False):
    ...
    half = int(size/2)
    if size % 2 != 0:
        lats = fp_full.lat.values[idx_release_lat-half:idx_release_lat+half+1]
        lons = fp_full.lon.values[idx_release_lon-half:idx_release_lon+half+1]
    else:
...
def get_past_values(met_var, jump):
    ...
    past_values = np.concatenate((met_var[jump:,:], met_var[:-jump,:]), axis=1)
