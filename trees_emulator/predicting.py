...
class MakePredictions:
    ...
    def __init__(self, clfs, data, inputs, jump, coarsened=False):
        self.clfs = clfs
        self.data = data
        self.inputs = inputs
    ...
    def plot_footprint_single(self, date, size=10, coarsened_factor=1, fixed_cbar = False):
        """
        ...
        # (10,10) can be size, or self.truths[idx,:] sqrt
        c = ax.pcolormesh(self.data.fp_lons, self.data.fp_lats, np.reshape(self.predictions[idx,:], (size, size)), transform=cartopy.crs.PlateCarree(), cmap="Reds", vmax = vmax, vmin=0)
        #c = axp.pcolormesh(self.data.fp_lons, self.data.fp_lats, np.reshape(self.predictions[idx,:], (10,10)), transform=cartopy.crs.PlateCarree(), cmap="Reds", vmax = vmax, vmin=0)
        ...
        if coarsened_factor == 1:
            ax.set_title("Emulator-generated footprint - "+ self.data.site + "\n" + date.strftime("%m/%d/%Y, %H:00"), fontsize = 17)
        else:
            ax.set_title("Emulator-generated footprint - "+ self.data.site + "\n" + date.strftime("%m/%d/%Y, %H:00") + ", " + "(coarsened " + str(coarsened_factor) + "x)", fontsize = 17)
    ...
    def plot_footprint_single_withOG(self, date, size=10, coarsened_factor=1, OG=None):
        if OG == None: print("You need to pass an OG resolution for the center...")

        else:
            ax, point = self.plot_footprint_single(date, size, coarsened_factor)
            ...
            if type(date) == str:
                ...
                try:
                    idx = pd.DatetimeIndex(OG.data.met.time.values[OG.jump:-3]).get_loc(date)
                except:
                    raise KeyError("This date is out of range")
            ...
            if type(date) == int:
                idx = date
                try:
                    date = pd.DatetimeIndex(OG.data.met.time.values[OG.jump:-3])[idx]
                except:
                    raise KeyError("This index is out of range")

            vmax = np.nanmax(OG.truths[idx,:])

            inset_width = fig_width / coarsened_factor
            inset_height = fig_height / coarsened_factor

            print(point[0].get_data())

            # Create a new GeoAxes object for the inset axis
            inset_ax = ax.figure.add_subplot(projection=cartopy.crs.PlateCarree(),
                                             position=[OG.data.fp_lons[0], OG.data.fp_lats[0], inset_width, inset_height])
            inset_ax.set_extent([OG.data.fp_lons[0], OG.data.fp_lons[-1], 
                                 OG.data.fp_lats[0], OG.data.fp_lats[-1]],
                                 crs=cartopy.crs.PlateCarree())
            
            inset_ax.pcolormesh(OG.data.fp_lons, OG.data.fp_lats, np.reshape(OG.predictions[idx,:], (size, size)), transform=cartopy.crs.PlateCarree(), cmap="Reds", vmax = vmax, vmin=0)


            # Set the extent for the inset axis
            #inset_ax.set_extent(OG_area, crs=cartopy.crs.PlateCarree())  

            # lon_formatter = LongitudeFormatter(number_format='.1f', degree_symbol='', dateline_direction_label=True)
            # inset_ax.xaxis.set_major_formatter(lon_formatter)  
            # lat_formatter = LatitudeFormatter(number_format='.1f',  degree_symbol='',)
            # inset_ax.yaxis.set_major_formatter(lat_formatter) 
